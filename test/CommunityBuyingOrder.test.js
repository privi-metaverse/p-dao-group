...
contract("CommunityBuyingOrder", (accounts) => {
    ...
    var communitybuyingorder_contract;
    ...
    before(async () => {
        ...
        communitybuyingorder_contract = await CommunityBuyingOrder.new(
            prividao_contract.address,
    ...
    describe("PlaceBuyingOrderProposal", () => {
        it("not working if price is zero", async () => {
            ...
            await offertoken_contract.approve(communitybuyingorder_contract.address, 20, {from: accounts[7]});
            await offertoken_contract.approve(erc20tokenexchange_contract.address, price * amount, {from: accounts[0]})

            let thrownError;

            try {
                await communitybuyingorder_contract.PlaceBuyingOrderProposal({
                    communityId: communityId,
        ...
        it("not working if not member", async () => {
            const communityId = await prividao_contract.getCommunityIdByIndex(0);
            const exchangeId = 1;
            const offerId = 0;
            const offerTokenAddress = offertoken_contract.address;
            ...
            assert.include(thrownError.message, "just community members can create buyorder proposal.");
        })
        ...
        it("works well", async () => {
            ...
            const prevCount = await communitybuyingorder_contract.getBuyingOrderProposalCount();
            ...
            const count = await communitybuyingorder_contract.getBuyingOrderProposalCount();
            assert.equal(count.sub(prevCount).toNumber(), 1);
        })
    })

    describe("VoteBuyingOrderProposal", () => {
        it("not working if not founder", async () => {
            const buyingOrderProposalId = await communitybuyingorder_contract.getBuyingOrderProposalIds(0);
            const buyingOrderProposal = await communitybuyingorder_contract.getBuyingOrderProposal(buyingOrderProposalId.toString());

            await offertoken_contract.approve(communitybuyingorder_contract.address, 20, {from: accounts[9]});
            let thrownError;

            try {
                await communitybuyingorder_contract.VoteBuyingOrderProposal({
                    proposalId: buyingOrderProposal.proposalId,
                    communityId: buyingOrderProposal.communityId,
                    decision: true
                }, { from: accounts[4] });
            } catch(error) {
                thrownError = error;
        ...
        it("works well", async () => {
            const buyingOrderProposalId = await communitybuyingorder_contract.getBuyingOrderProposalIds(0);
            const buyingOrderProposal = await communitybuyingorder_contract.getBuyingOrderProposal(buyingOrderProposalId.toString());
            const prevBalance = await offertoken_contract.balanceOf(accounts[0]);
            ...
            const balance = await offertoken_contract.balanceOf(accounts[0]);

            assert.equal(prevBalance.sub(balance).toNumber(), 20);
        })
    })

    describe("cancelBuyingOrderProposal", () => {
        it("works well", async () => {
            const communityId = await prividao_contract.getCommunityIdByIndex(0);
            const exchangeId = 1;
            const offerId = 0;
            ...
            const buyingOrderProposalId = await communitybuyingorder_contract.getBuyingOrderProposalIds(1);
            const buyingOrderProposal = await communitybuyingorder_contract.getBuyingOrderProposal(buyingOrderProposalId.toString());

            const prevBalance = await offertoken_contract.balanceOf(accounts[7]);

            await communitybuyingorder_contract.cancelBuyingOrderProposal({
                proposalId: buyingOrderProposal.proposalId,
                communityId: buyingOrderProposal.communityId,
            }, { from: accounts[0] });

            const balance = await offertoken_contract.balanceOf(accounts[7]);
            assert.equal(balance.sub(prevBalance).toNumber(), amount);
        })
    })
})